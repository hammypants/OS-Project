...
namespace OS_PROJECT
{
    ...
    class CPU
    {
        ...
        int id;
        int processCounter;
        Stopwatch totalElapsedTimeForCPU = new Stopwatch();
        Stopwatch totalElapsedTimeForProcess = new Stopwatch();
        ...
        public void Run()
        {
            while (true)
            {
                ...
                totalElapsedTimeForCPU.Stop();
                Console.WriteLine("Elapsed time for CPU " + id + " to run " + processCounter + " processes was " + totalElapsedTimeForCPU.Elapsed.TotalMilliseconds.ToString() + ".");
            }
        }
        ...
        void CheckReadyQueue()
        {
            while (currentProcess == null)
            {
                lock (rqLock)
                {
                    ...
                    {
                        totalElapsedTimeForProcess.Start();
                        currentProcess = RQ.AccessQueue.Dequeue();
        ...
        {
            totalElapsedTimeForProcess.Stop();
            Console.WriteLine("Elapsed time for process " + currentProcess.PCB.ProcessID + " is " + totalElapsedTimeForProcess.Elapsed.TotalMilliseconds.ToString() + ".");
            SaveProcessStatus();
            Console.WriteLine("Process " + cpuPCB.ProcessID + " used " + cpuPCB.IoCount + " I/O calls.");
            SystemCaller.WriteToFile("Process " + cpuPCB.ProcessID + " used " + cpuPCB.IoCount + " I/O calls.");
            currentProcess = null;
            CheckForNewProcess();
