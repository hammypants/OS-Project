...
namespace OS_PROJECT
{
    class SystemCaller
    {
        ...
        {
            for (int iterator = 0; iterator < r.GetMemorySize() - 1900; iterator++)
            {
        ...
        public static void WriteRAMToFile(RAM r)
        {
            ...
            {
                StreamWriter file = File.AppendText("osProjMem"+timesWritten+".txt");

                for (int iterator = 0; iterator < r.GetMemorySize(); iterator++)//for (int iterator = 0; iterator < r.GetMemorySize() - 1900; iterator++)
                {
                    uint _uint = r.ReadDataFromMemory((uint)iterator);
                    string _data = String.Format("{0:X}", _uint);
                ...
                //int count = 0;
                //for (int i = 0; i < r.GetMemorySize(); i++)
                //{
                //    file.WriteLine();
                //    count++;
                //}
